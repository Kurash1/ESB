...
defined_text = {
	...
	using "classes/countries/nordic_clans.class" as nordic_clan
	foreach nordic_clan = {
		where = {
			has_key = domesday
		}
		var $tag$ = nordic_clan.id
		defineloc §"{$tag$}_startup_text" = nordic_clan.domesday
		text = {
			localisation_key = §"{$tag$}_startup_text"
			trigger = {
				tag = $tag$
			}
		}
